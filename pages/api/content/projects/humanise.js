...
const humanizeText = async (text) => {
  // Define the prompt to instruct GPT for paraphrasing
  const prompt = `
You are an expert content writer. Paraphrase the following text to make it more human-friendly and readable while preserving its original meaning and formatting, including any HTML tags or code blocks. Do not give anything else except the paraphrased content.

Text to paraphrase:
${text}
`;
console.log("Request", humanizeText);
  try {
    const response = await openai.chat.completions.create({
      model: 'gpt-4', // You can use 'gpt-4' or any available model
